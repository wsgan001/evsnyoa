...
<script type="text/javascript">
	/*找到发件人*/
	function findCLSJEmailAddress(){
		var data = "eventID="+'${eventID}';
		$.post('<%=basePath%>sendmail/findCLSJEmailAddress.action',data,function(result){
			if(result!=null){$("#recipients").val(result);}
			else {alert("获取内容失败！");}
		});
	}
	/* 找到其他抄送人 */    
	function findSHManWithKGManEmailAddress() {
		var data = "eventID="+'${eventID}';
		$.post('<%=basePath%>sendmail/findSHManWithKGManEmailAddress.action',data,function(result){
	...
	/*找到设备内容*/
	function findKGEmailContent() {
		var data = "eventID="+${eventID};
		$.post('<%=basePath%>sendmail/findKGEmailContent.action',data,function(result){
								...
								<s:iterator value="deviceList" status="item">
									<tr>
									   ...
									   <td >			
										...
										<td >
											<!-- Icons -->
											 <a href="<%=basePath%>device/loadDeviceToAdEdit.action?deviceID=<s:property value="id" />&eventID=${eventID}" title="编辑">编辑<img
			...
			<div class="tab-content" id="tab2">
					<div style="margin-left:20%;width: 800px;">
						<form >
							<fieldset>
								<p>
									<label for="recipients">收件人(执行人):</label><input type="button"
										value="获取邮件收件人" id="findMailrecipients"
										name="findMailrecipients" onclick="findCLSJEmailAddress()" />
								...
								<p>
									<label for="copyToMan">抄送(相关负责人):</label><input type="button"
										...
										name="findMailCopyToMan" onclick="findSHManWithKGManEmailAddress()" />
