		...
		<div class="content-box">
           ...
           <h3>事件分配管理</h3>
				<ul class="content-box-tabs">
					<li><a href="#tab1" class="default-tab">事件分配</a>
					</li>
				</ul>
				<div class="clear"></div>
			</div>
									...
									<tr>	
									   ...
									   <td >			
										<s:property value="eventType"/>
										 ...
										 <td >			
										<s:property value="accountByDjsjId.name"/>
										</td>
										 <td >			
										<s:if test='%{dispatchState==0}'>未分配</s:if>
										<s:if test='%{dispatchState==1}'>已分配</s:if>
										</td>
										 <td >
										...
										<td >
											<!-- Icons -->
											 <a href="<%=basePath%>event/loadEventToDispatch.action?eventID=<s:property value="id" />&page=<s:property value="page"/>" title="分配">分配<img
												src="<%=basePath%>admin/resources/images/icons/information.png"
												alt="分配" style="vertical-align:middle;"/> </a> 
										</td>
									</tr>
								</s:iterator>
						   </tbody>
						   <tfoot>
							<tr>
								...
								/<s:property value="pageBean.totalPage" />&nbsp;页，
										<a href="<%=basePath%>event/loadUndispatchEvent.action?page=1"
											title="首页">&laquo;首页</a>
										...
										<s:else>
											<a
												href="<%=basePath%>event/loadUndispatchEvent.action?page=<s:property value="%{pageBean.currentPage-1}"/>">&laquo;上一页</a>
										</s:else>										
										<!--  <a href="#"
										...
										<s:else>
											<a
												href="<%=basePath%>event/loadUndispatchEvent.action?page=<s:property value="%{pageBean.currentPage+1}"/>">下一页&raquo;</a>
										</s:else>
										<a
											href="<%=basePath%>event/loadUndispatchEvent.action?page=<s:property value="pageBean.totalPage"/>">尾页&raquo;</a>
									</div> <!-- End .pagination -->
									<div class="clear"></div>
								</td>
							</tr>
