	...
	//事件类型数组
	 var eventTypeArr = new Array(['1','现场交流'],['1','调研'],['1','解决方案'],['1','招标参数'],['1','投标文件'],['1','市场会议'],['1','培训'],['1','演示'],['1','测试'],['1','提交用户文档'],['1','报价']
	  ...
	  $(function(){
		  ...
		  $('input:radio[name="periodicEvent"]').change(function(){//这样才能选择到radio每一个name=periodicEvent的事件
			 /* var $periodiceventdiv = $("#periodiceventdiv"); */
			 var $periodiceventdiv = $(".periodiceventdiv");
			 var $eventdatediv = $(".eventdatediv");
			 var eventValue = $('input:radio[name="periodicEvent"]:checked').val();
			 if(eventValue=="0"){
				 //执行普通事件
				 $periodiceventdiv.hide();
				 $eventdatediv.show();
			 }else if(eventValue=="1"){
				 //执行周期性事件
				 $periodiceventdiv.show();
				 $eventdatediv.hide();
			 }
		  }); 
	  });
	 ...
	 function checkIsPeriodic(){
		  ...
		  if(eventValue=="1"){
			  ...
			  var periodicEvent = checkPeriodicInfo();
				  ...
				  $("#eventForm").attr("action",'<%=basePath%>periodicevent/addPeriodicEvent.action'); 
				  return true;
			  }
			  return false;
			...
			<div class="content-box-content">
				<div class="tab-content default-tab" id="tab1">
					...
					<div style="margin-left:40%;">
						<form action="<%=basePath%>event/add_event.action" method="post" enctype="multipart/form-data" name="eventForm" id="eventForm" onsubmit="return checkIsPeriodic()">
							<%-- <p>
							...
							<fieldset>
								<div>
									<p>
										...
										<s:textfield name="eventCustomer" cssClass="text-input"
											id="eventCustomer" class="eventCustomer" size="30" onkeyup="eventCustomerChange()"></s:textfield>
									</p>
								</div>
								...
								<div>
									 <p>
										<label for="periodicEvent">事&nbsp;件&nbsp;类&nbsp;型:</label>
										<input type="radio" name="periodicEvent" id="periodicEvent" value="0" checked><label for="periodicEvent">不是周期性事件</label>	
							 			<input type="radio" name="periodicEvent" id="periodicEvent" value="1"><label for="periodicEvent">是周期性事件</label>
									...
									<p>
										...
										<s:textfield name="eventStartTime" cssClass="text-input"
											...
											onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" onchange="checkStartTime()" ></s:textfield>
									</p>
								</div>
								<div class="periodiceventdiv">
									<p>
								...
								<div class="periodiceventdiv">
									<p>
										...
										<select id="intervalType" name="intervalType"  cssClass="small-input" onchange="intervalTimeChange(this.value)">
										...
										<input type="hidden" id="timeInterval" name="timeInterval" value="0"/>
									</p>
								</div>
								<div class="periodiceventdiv">
								<p>
								...
								<div style="margin-top: 10px" class="eventdatediv">
									<p>
